package com.maksboss800.billing
...
class BillingManager(val activity:AppCompatActivity) {
    ...
    //подгтовка информации про нашу покупку
    private fun getItem(){
    ...
    //подтверждаєм покупку
    //purchase клас який несе інформацію про покупку
    private fun nonConsumableItem(purchase:Purchase){
        if(purchase.purchaseState==Purchase.PurchaseState.PURCHASED){
            if(!purchase.isAcknowledged){
                //потверждение покупки
                val acParams=AcknowledgePurchaseParams.newBuilder()
                    .setPurchaseToken(purchase.purchaseToken).build()
                    //purchaseTokeb- id нашої покупки
                bClient?.acknowledgePurchase(acParams){
                    if (it.responseCode == BillingClient.BillingResponseCode.OK) {

                        savePref(true)
                        Toast.makeText(activity,"Thanks for your purchase",Toast.LENGTH_LONG).show()
                    }else{
                        savePref(false)
                        Toast.makeText(activity,"Problems with purchase",Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
